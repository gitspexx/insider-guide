...
// Hoisted out of LocationFilter: declaring it inside the parent created a new
// component identity on every render, remounting all buttons (losing the
// whileTap animation state and thrashing the DOM). `active`/`onChange` flow in
// as props instead.
function FilterButton({ value, label, count, active, onChange }) {
...
export default function LocationFilter({ locations, counts, active, onChange }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <FilterButton value="all" label="All Cities" count={total} active={active} onChange={onChange} />
      {locations.map((loc) => (
        <FilterButton key={loc} value={loc} label={loc} count={counts?.[loc]} active={active} onChange={onChange} />
      ))}
    </div>
  )
}
